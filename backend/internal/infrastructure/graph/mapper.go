...
func mapMatchHistoryEntry(entry services.MatchHistoryEntry) *model.MatchHistoryEntry {
	finalScores := make([]*model.TeamScore, 0, len(entry.FinalScores))
	for teamID, points := range entry.FinalScores {
		finalScores = append(finalScores, &model.TeamScore{
			TeamID: teamID,
			Points: int32(points),
		})
	}
	sort.Slice(finalScores, func(i, j int) bool {
		return finalScores[i].TeamID < finalScores[j].TeamID
	})

	return &model.MatchHistoryEntry{
		GameID:        entry.GameID,
		RoomID:        entry.RoomID,
		PlayedAt:      entry.PlayedAt,
		Won:           entry.Won,
		WinnerTeamID:  entry.WinnerTeamID,
		MyTeamID:      entry.MyTeamID,
		MyScore:       int32(entry.MyScore),
		OpponentScore: int32(entry.OpponentScore),
		FinalScores:   finalScores,
	}
}

func mapGameReplay(replay *services.GameReplay) *model.GameReplay {
	if replay == nil {
		return nil
	}

	events := make([]*model.ReplayEvent, 0, len(replay.Events))
	for _, e := range replay.Events {
		events = append(events, &model.ReplayEvent{
			ID:        e.ID,
			Type:      e.Type,
